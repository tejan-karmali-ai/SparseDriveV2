...
class MotionPlanningHeadV1(BaseModule):
    def __init__(
        self,
        fut_ts=12,
        fut_mode=6,
        ...
        super(MotionPlanningHeadV1, self).__init__()
        self.fut_ts = fut_ts
    ...
    def forward(
        ...
        plan_mode_query = self.plan_mode_query.weight[None, None]
        ...
        for i, op in enumerate(self.operation_order):
            if self.layers[i] is None:
                ...
                plan_query = plan_mode_query + (instance_feature + anchor_embed)[:, num_anchor:].unsqueeze(2) + target_embedding
                (
                    motion_cls,
                    motion_reg,
                    plan_cls,
