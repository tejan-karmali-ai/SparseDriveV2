...
def main():
    ...
    os.system(f"cp {args.config} {cfg.work_dir}")
    ...
    data = datasets[0].__getitem__(150)
    ...
    #     if i % 100 == 0:
    #         print(i)
    #     data = datasets[0].disappearing_objects(i)
    #     if data is not None and len(data) <= 20:
    #         cnt += 1
    #         idxs.append(i)
    #         # if len(idxs) > 0 and i != idxs[-1] + 1:
    #         #     idxs.append(idxs[-1]+1)
    #         #     idxs.append(idxs[-1]+1)
    # import ipdb; ipdb.set_trace()


    # data = datasets[0].__getitem__(8070) ## reverse
    # data = datasets[0].__getitem__(188040) ## lidar/ego
    ...
    #     os.system(f"mkdir {cfg.work_dir}/{model_tag}")
    #     os.system(f"cp {args.config} {cfg.work_dir}/{model_tag} ")
    #     latest = max(glob.glob(f'{cfg.work_dir}/iter_*.pth'),
    #                 key=lambda p: int(re.search(r'iter_(\d+)\.pth$', os.path.basename(p)).group(1)))
    #     os.system(f"cp {latest} {cfg.work_dir}/{model_tag} ")
