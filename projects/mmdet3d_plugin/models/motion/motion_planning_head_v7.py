...
class MotionPlanningHeadV7(BaseModule):
    def __init__(
        self,
        fut_ts=12,
        fut_mode=6,
        ego_fut_mode=18,
        ego_fut_mode_refine=18,
        motion_anchor=None,
        ...
        super(MotionPlanningHeadV7, self).__init__()
        self.fut_ts = fut_ts
        self.fut_mode = fut_mode
        self.ego_fut_mode = ego_fut_mode
        self.ego_fut_mode_refine = ego_fut_mode_refine
        ...
        self.op_config_map = {
            ...
            "ego_temp_gnn": [temp_graph_model, ATTENTION],
            "mode_temp_gnn": [temp_graph_model, ATTENTION],
            "gnn": [graph_model, ATTENTION],
            "ego_gnn": [graph_model, ATTENTION],
            "mode_ins_gnn": [graph_model, ATTENTION],
            "cross_gnn": [cross_graph_model, ATTENTION],
            "ego_cross_gnn": [cross_graph_model, ATTENTION],
            "mode_cross_gnn": [cross_graph_model, ATTENTION],
            ...
            "ego_norm": [norm_layer, NORM_LAYERS],
            "mode_norm": [mode_norm_layer, NORM_LAYERS],
            "ffn": [ffn, FEEDFORWARD_NETWORK],
            "ego_ffn": [ffn, FEEDFORWARD_NETWORK],
            "mode_ffn": [ffn, FEEDFORWARD_NETWORK],
    ...
    def forward(
        ...
        temp_ego_anchor_embed = temp_anchor_embed[:, -1:].flatten(0, 1)       
        temp_ego_feature = temp_instance_feature[:, -1:].flatten(0, 1)
        temp_ego_mask = temp_mask[:, -1:].flatten(0, 1)
        temp_instance_feature = temp_instance_feature[:, :-1].flatten(0, 1)
        temp_anchor_embed = temp_anchor_embed[:, :-1].flatten(0, 1)
        temp_mask = temp_mask[:, :-1].flatten(0, 1)
        ...
        plan_anchor_embed = ego_anchor_embed.repeat(1, self.ego_fut_mode, 1)
        # =========== encoder condition ===========
        ...
        for i, op in enumerate(self.operation_order):
            if op == "temp_gnn":
                ...
                instance_feature = instance_feature.reshape(bs, num_anchor, dim)
            elif op == "ego_temp_gnn":
                ego_feature = self.graph_model(
                    i,
                    ego_feature.flatten(0, 1).unsqueeze(1),
                    temp_ego_feature,
                    temp_ego_feature,
                    query_pos=ego_anchor_embed.flatten(0, 1).unsqueeze(1),
                    key_pos=temp_ego_anchor_embed,
                    key_padding_mask=temp_ego_mask,
                )
                ego_feature = ego_feature.reshape(bs, 1, dim)
            elif op == "mode_temp_gnn":
                num_mode = plan_query.shape[1]
                temp_ego_feature_ = temp_ego_feature.unsqueeze(1).repeat(1, num_mode, 1, 1).flatten(0, 1)
                temp_ego_anchor_embed_ = temp_ego_anchor_embed.unsqueeze(1).repeat(1, num_mode, 1, 1).flatten(0, 1)
                temp_ego_mask_ = temp_ego_mask.unsqueeze(1).repeat(1, num_mode, 1).flatten(0, 1)
                plan_query = self.graph_model(
                    i,
                    plan_query.flatten(0, 1).unsqueeze(1),
                    temp_ego_feature_,
                    temp_ego_feature_,
                    query_pos=plan_anchor_embed.flatten(0, 1).unsqueeze(1),
                    key_pos=temp_ego_anchor_embed_,
                    key_padding_mask=temp_ego_mask_,
                )
                plan_query = plan_query.reshape(bs, num_mode, dim)
            elif op == "gnn":
                instance_feature = self.graph_model(
                    i,
                    instance_feature,
                ...
                ego_feature = self.graph_model(
                    ...
                    instance_feature_selected,
                    instance_feature_selected,
                    query_pos=ego_anchor_embed,
                    key_pos=anchor_embed_selected,
                )
            elif op == "mode_ins_gnn":
                plan_query = self.graph_model(
                    i,
                    plan_query,
                    instance_feature_selected,
                    instance_feature_selected,
                    query_pos=plan_anchor_embed,
                    key_pos=anchor_embed_selected,
                )
            elif op == "norm" or op == "ffn":
                instance_feature = self.layers[i](instance_feature)
            elif op == "ego_norm" or op == "ego_ffn":
                ego_feature = self.layers[i](ego_feature)
            elif op == "mode_gnn" or op == "mode_norm" or op == "mode_ffn":
            ...
            elif op == "ego_cross_gnn":
                ego_feature = self.layers[i](
                    ego_feature,
                    key=map_instance_feature_selected,
                    query_pos=ego_anchor_embed,
                    key_pos=map_anchor_embed_selected,
                )
            elif op == "mode_cross_gnn":
        ...
        self.instance_queue.cache_motion(instance_feature, det_output, metas)
        status = plan_result.get("ego_status_reg")
        self.instance_queue.cache_planning(ego_feature, status)
    ...
    def loss_planning(self, model_outs, data):
        ...
        if self.match_reference_group is not None:
            ...
            mode_idx, dist = self.planning_sampler.get_mode_idx(reg, gt_reg, gt_mask, data)
        else:
            mode_idx = dist = None
        ...
        for key, value in self.plan_config.items():
            for decoder_idx, planning_result in enumerate(planning_results):
                if "temporal" in key or "spatial" in key:
                    ...
                    cls = planning_result[key + "_" + "cls"][:, 0].sigmoid()
                    gt_cls = data[f"gt_{key}"]
                    gt_cls = F.one_hot(gt_cls, num_classes=40).float()
                    cls_loss = F.binary_cross_entropy(
                        cls, gt_cls
                    )
                    output[f"planning_{key}_cls_loss_{decoder_idx}"] = cls_loss * value["cls_weight"]
                elif "lateral_point" in key:
                    cls = planning_result[key + "_" + "cls"][:, 0].softmax(dim=-1)
                    gt_cls = data[f"gt_{key}"]
                    gt_cls = F.one_hot(gt_cls, num_classes=15).float()
                    cls_loss = F.binary_cross_entropy(
                        cls, gt_cls
                    )
                    output[f"planning_{key}_cls_loss_{decoder_idx}"] = cls_loss * value["cls_weight"]
