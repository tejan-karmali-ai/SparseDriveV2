...
class MotionPlanningHeadV3(BaseModule):
    def __init__(
        self,
        fut_ts=12,
        fut_mode=6,
        ...
        super(MotionPlanningHeadV3, self).__init__()
        self.fut_ts = fut_ts
    ...
    def loss_planning(self, model_outs, data):
        ...
        output = {}
        for decoder_idx, (cls, col_cls, reg, status, path) in enumerate(
            zip(cls_scores, col_cls_scores, reg_preds, status_preds, path_preds)
        ):
            (
                cls,
                cls_target, 
            ...
            if col_cls is not None:
                ...
                col_cls_target = torch.logical_not(data["collision"]).long()
