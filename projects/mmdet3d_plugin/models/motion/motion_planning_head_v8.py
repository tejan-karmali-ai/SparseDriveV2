...
class MotionPlanningHeadV8(BaseModule):
    def __init__(
        self,
        fut_ts=12,
        fut_mode=6,
        ...
        super(MotionPlanningHeadV8, self).__init__()
        self.fut_ts = fut_ts
        ...
        if col_plan_loss_cls is not None:
    ...
    def loss_planning(self, model_outs, data):
        ...
        for decoder_idx, planning_result in enumerate(planning_results):
            ...
            if col_cls is not None:
                col_cls_loss = self.col_plan_loss_cls(col_cls, col_label, weight=col_weight)
                output[f"planning_col_cls_loss_{decoder_idx}"] = col_cls_loss * self.plan_config["col"]["cls_weight"]

        # print("\n", lat_cls_loss.item(), "  \n")
        # print("   ", lat_cls.max().item(), "   ")
            # print(torch.isnan(lat_cls_loss).any() or torch.isinf(lat_cls_loss).any())
        # print("   ", lat_cls_weight.max(), lat_cls_weight.min(), torch.isnan(lat_cls_weight).any() or torch.isinf(lat_cls_weight).any(), "   ")
        return output
