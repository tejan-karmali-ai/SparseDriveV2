...
class MotionPlanningHeadV4(BaseModule):
    def __init__(
        self,
        fut_ts=12,
        fut_mode=6,
        ...
        super(MotionPlanningHeadV4, self).__init__()
        self.fut_ts = fut_ts
    ...
    def forward(
        ...
        planning_col_classification = []
        planning_road_classification = []
        planning_prediction = []
        planning_status = []
        paths = []
        plan_query = None
        for i, op in enumerate(self.operation_order):
            if self.layers[i] is None:
                ...
                if self.use_route and self.route_cross_attn:
                    route_embedding = self.route_encoder(metas["route"].float())[:, None]
                ...
                if plan_query is None:
                    if self.use_tp:
                        target_embedding = self.target_encoder(metas["tp_near"].float())[:, None, None]
                    else:
                        target_embedding = 0
                    if self.use_route and not self.route_cross_attn:
                        route_embedding = self.route_encoder(metas["route"].float())[:, None, None]
                    else:
                        route_embedding = 0
                ...
                planning_col_classification.append(plan_col_cls)
                planning_road_classification.append(plan_road_cls)
                ...
                plan_anchor = plan_reg.cumsum(dim=-2).reshape(bs, self.ego_fut_mode, -1)
                refined_plan_query = self.layers[i](
                    plan_query[:,0],
                    plan_anchor,
                    None,
                    feature_maps,
        ...
        planning_output = {
            ...
            "col_classification": planning_col_classification,
            "road_classification": planning_road_classification,
    ...
    def loss_planning(self, model_outs, data):
        if "croase2fine" in self.operation_order:
            ...
            cls_scores = model_outs["classification"]
            reg_preds = model_outs["prediction"]
        col_cls_scores = model_outs["col_classification"]
        road_cls_scores = model_outs["road_classification"]
        status_preds = model_outs["status"]
        path_preds = model_outs["paths"]
        output = {}
        for decoder_idx, (cls, col_cls, road_cls, reg, status, path) in enumerate(
            zip(cls_scores, col_cls_scores, road_cls_scores, reg_preds, status_preds, path_preds)
        ):
            (
                cls,
                cls_target, 
            ...
            if col_cls is not None:
                col_cls = col_cls[:, 0, :, None]
                col_cls_target = data["collision"].long()
                col_cls = col_cls.flatten(end_dim=1)
                col_cls_target = col_cls_target.flatten(end_dim=1)
                col_cls_loss = self.plan_loss_cls(col_cls, col_cls_target, avg_factor=bs * self.ego_fut_mode)
                output.update({f"planning_loss_col_cls_{decoder_idx}": col_cls_loss,})
            if road_cls is not None:
                road_cls = road_cls[:, 0, :, None]
                road_cls_target = torch.logical_not(data["on_road"]).long()
                road_cls = road_cls.flatten(end_dim=1)
                road_cls_target = road_cls_target.flatten(end_dim=1)
                road_cls_loss = self.plan_loss_cls(road_cls, road_cls_target, avg_factor=bs * self.ego_fut_mode)
                output.update({f"planning_loss_road_cls_{decoder_idx}": road_cls_loss,})
