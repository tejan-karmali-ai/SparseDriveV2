...
class BEVRender:
    ...
    def render(
        ...
        if self.plot_choices["bev_gt"]:
            ...
            # self.draw_planning_gt_vis_point_idx(data)
    ...
    def draw_detection_pred(self, result):
        ...
        for i in range(result['labels_3d'].shape[0]):
            ...
            if score < SCORE_THRESH: 
                continue
            if self.plot_choices["det_attn_weight"] and "det_attn_weight" in result:
                weight = result['det_attn_weight'][i]
                v_min = result['det_attn_weight'].min()
                v_max = result['det_attn_weight'].max()
                weight = np.clip((weight - v_min) / (v_max - v_min + 1e-8), 0, 1)
                color = cmap(weight)[:3]
            else:
    ...
    def draw_map_pred(self, result):
        ...
        for i in range(result['scores'].shape[0]):
            ...
            if  score < MAP_SCORE_THRESH:
                continue
            if self.plot_choices["map_attn_weight"] and "map_attn_weight" in result:
                weight = result['map_attn_weight'][i]
                v_min = result['map_attn_weight'].min()
                v_max = result['map_attn_weight'].max()
                weight = np.clip((weight - v_min) / (v_max - v_min + 1e-8), 0, 1)
                color = cmap(weight)[:3]
            else:
    ...
    def draw_planning_gt_old_traj(self, data):
    ...
    def draw_planning_gt(self, data):
        if not self.plot_choices['planning']:
            return
        # draw planning gt
        masks = data['gt_traj_mask'].astype(bool)
        if masks[0] != 0:
            plan_traj = data['gt_traj'][masks]
            plan_traj[abs(plan_traj) < 0.01] = 0.0
            # plan_traj = plan_traj.cumsum(axis=0)
            plan_traj = np.concatenate((np.zeros((1, plan_traj.shape[1])), plan_traj), axis=0)
            # self.axes.plot(plan_traj[:,0], plan_traj[:,1], color="yellow", linewidth=40, marker='o', linestyle='-', markersize=50)
            # self._render_traj(plan_traj, traj_score=1.0,
            #     colormap='winter', dot_size=70)
            self.axes.plot(plan_traj[:, 0], plan_traj[:, 1], color="b", linewidth=8, marker="o", markersize=16)


    def draw_planning_gt_dist(self, data):
        if not self.plot_choices['planning']:
            return

        key = data["planning_key"]
        path = data["gt_" + key]
        mask = data["gt_" + key + "_mask"]
        if mask.sum() == 0:
            return
        path = path[mask.astype(np.bool)]
        path_cat = np.concatenate((np.zeros((1, 2)), path), axis=0)
        anchor = np.load("data/kmeans/path_1m_1024_b2d.npy")

        r_max = anchor.max(axis=0)
        r_min = anchor.min(axis=0)
        delta = np.abs(anchor - path)
        delta_norm = (delta - r_min) / (r_max - r_min)
        dist = np.linalg.norm(delta_norm, axis=-1)
        dist = dist * mask
        dist = dist.mean(axis=-1)

        # point_weight = [1.0] * 5 + [0.6] * 5 + [0.4] * 5
        # point_weight = [14] * 3 + [6.36] * 3 + [2.7] * 3 + [2.0] * 2 + [1.36] * 2 + [0.7] * 2
        # dist *= np.array(point_weight)
        # dist = np.linalg.norm(anchor - path, axis=-1)
        # dist = dist * mask
        # dist = dist.mean(axis=-1)

        idx = np.argsort(dist)[:100]          # 100 个索引
        top_dist = dist[idx]                  # 100 个距离
        top_path = anchor[idx]                  # 100×15×2

        # 2. 距离 → 0~1
        norm = plt.Normalize(top_dist.min(), top_dist.max())
        cmap = plt.cm.get_cmap('Reds')       # 越深越黑；想反序用 'Greys_r'

        # 3. 画图
        for d, pts in zip(top_dist, top_path):
            color = cmap(norm(d))
            self.axes.plot(pts[:, 0], pts[:, 1], color=color, linewidth=3)
        self.axes.plot(path_cat[:, 0], path_cat[:, 1], color="b", linewidth=3, marker='o', linestyle='-', markersize=7)
        # 4. 可选：色条
        # sm = plt.cm.ScalarMappable(cmap=cmap, norm=norm)
        # sm.set_array([])
        # self.axes.colorbar(sm, label='distance')
        # plt.gca().set_aspect('equal')
        # plt.title('100 closest paths')
        # plt.show()
    def draw_path(self, data):
        masks = data['gt_spatial_mask'].astype(bool)
        if masks[0] == 0:
            return
        plan_traj = data['gt_spatial'][masks]
        plan_traj[abs(plan_traj) < 0.01] = 0.0
        self.axes.plot(plan_traj[:, 0], plan_traj[:, 1], color="r", linewidth=8, marker="o", markersize=16)

        masks = data['gt_spatial_ego_mask'].astype(bool)
        if masks[0] == 0:
            return
        plan_traj = data['gt_spatial_ego'][masks]
        plan_traj[abs(plan_traj) < 0.01] = 0.0
        self.axes.plot(plan_traj[:, 0], plan_traj[:, 1], color="g", linewidth=8, marker="o", markersize=16)

        masks = data['gt_spatial_hip_mask'].astype(bool)
        if masks[0] == 0:
            return
        plan_traj = data['gt_spatial_hip'][masks]
        plan_traj[abs(plan_traj) < 0.01] = 0.0
        self.axes.plot(plan_traj[:, 0], plan_traj[:, 1], color="b", linewidth=8, marker="o", markersize=16)

    def draw_planning_gt_vis_point_idx(self, data):
        if not self.plot_choices['planning']:
            return
        # draw planning gt
        masks = data['gt_spatial_1m_mask'].astype(bool)
        if masks[0] == 0:
            return
        plan_traj = data['gt_spatial_1m'][masks]
        plan_traj[abs(plan_traj) < 0.01] = 0.0
        self.axes.plot(plan_traj[:, 0], plan_traj[:, 1], color="r", linewidth=8, marker="o", markersize=16)
        angle = data["angle"] / 180 * np.pi
        if angle >= 0:
            x = 1
        else:
            x = -1
        if np.abs(angle) < 1e-2:
            ...
            y = x / np.tan(angle)
        self.axes.plot([0, x], [0, y], color="blue", linewidth=8, marker="o", markersize=20)
        point_idx = data["gt_lateral_point_1m"]
        point = plan_traj[point_idx]
        self.axes.scatter(point[0], point[1], color="black", s=600)

        throttle = data["throttle"]
        brake = data["brake"]
        steer = data["steer"]
        steer_pid = np.degrees(np.pi / 2 - np.arctan2(point[1], point[0])) / 90 * 1.25
        steer_pid = np.round(steer_pid, 2)

        self.axes.text(-15, 25, f"throttle: {throttle}", fontsize=60, color='r')
        self.axes.text(-15, 22, f"steer: {steer}", fontsize=60, color='r')
        self.axes.text(-15, 19, f"steer_pid: {steer_pid}", fontsize=60, color='r')
        self.axes.text(-15, 16, f"brake: {brake}", fontsize=60, color='r')
    ...
    def draw_planning_pred_v2(self, data, result, top_k=6): # old, with cmd
        if self.plot_choices['track'] and "ego_anchor_queue" in result:
            ego_temp_bboxes = result["ego_anchor_queue"]
            ego_period = result["ego_period"]
            for j in range(ego_period[0]):
    ...
    def draw_path_gt(self, data):
        ...
        masks = data['gt_spatial_mask'].astype(bool)
        if masks[0] != 0:
            path = data["gt_spatial"][masks]
            x = np.array([pt[0] for pt in path])
            y = np.array([pt[1] for pt in path])
            self.axes.plot(x, y, color="r", linewidth=6, marker="o", markersize=12)
    ...
    def draw_speed_pred(self, data, result):
        if not (self.plot_choices['speed'] and "target_speed_05s_score" in result):
            return
        speed_intervals = list(range(0, 90, 2))
        speed_intervals = [x / 10 for x in speed_intervals]
        score = result["target_speed_05s_score"].numpy()
        # self.axes[j // 3, j % 3]
        ax = self.fig.add_axes([0.01, 0.80, 0.3, 0.2])
