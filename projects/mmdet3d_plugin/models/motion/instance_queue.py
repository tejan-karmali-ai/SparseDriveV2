...
from projects.mmdet3d_plugin.ops import feature_maps_format, deformable_format
...
class InstanceQueue(nn.Module):
    def __init__(
        ...
        self.history_aug = history_aug

        assert ego_init_mode in [0, 1, 2]
        self.ego_init_mode = ego_init_mode
        if ego_init_mode == 0:
        ...
        elif ego_init_mode == 1:
            self.ego_feature = nn.Embedding(1, embed_dims)
        elif ego_init_mode == 2:
            self.ego_feature = nn.Parameter(
                torch.zeros([1, embed_dims]),
                requires_grad=False,
            )
        ...
        self.use_ego_status = cond_config.get("ego_status") is not None
    ...
    def get(
        ...
        if self.history_aug:
            noise = torch.rand_like(temp_ego_anchor[..., :2]) - 0.5
            temp_ego_anchor[..., :2] += noise
    ...
    def prepare_planning(
        ...
        elif self.ego_init_mode == 1:
            ego_feature = self.ego_feature.weight[None].repeat(batch_size, 1, 1)
        elif self.ego_init_mode == 2:
            ego_feature = self.ego_feature[None].repeat(batch_size, 1, 1)
        ...
        if self.use_ego_status:
            ego_anchor[..., VY] = metas["ego_status"][..., 6].unsqueeze(-1)
