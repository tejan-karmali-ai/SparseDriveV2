...
class NewPlanMetric():
    def __init__(self, plan_config):
        ...
        for key, value in plan_config.items():
            self.statistic[key] = [0, 0]
    
    def update(self, key, result):
        self.statistic[key][0] += result
        self.statistic[key][1] += 1
...
def planning_eval_old(results, eval_config, logger):
    ...
    for i, data in enumerate(tqdm(dataloader)):
        ...
        pred_sdc_traj = res['img_bbox']['temporal_2hz_reg_final'].unsqueeze(0)
...
def planning_eval(results, eval_config, logger):
    ...
    dataloader = build_dataloader(
            dataset, samples_per_gpu=1, workers_per_gpu=1, shuffle=False, dist=False)
    plan_config = dataset.plan_config
    new_metric = NewPlanMetric(plan_config)
    for i, data in enumerate(tqdm(dataloader)):
        res = results[i]['img_bbox']
        for key, value in plan_config.items():
            if "spatial" in key or "temporal" in key:
                pred = res[key + "_reg_final"]
                gt = data["gt_" + key][0]
                mask = data["gt_" + key + "_mask"][0]
                if not mask.all():
                    continue
                L2 = (torch.norm(pred-gt, dim=-1) * mask).sum().item()
                new_metric.update(key, L2)
            elif "speed" in key:
                pred = res[key]
                gt_cls = data["gt_" + key][0]
                gt_reg = (gt_cls * torch.tensor(value["speed_intervals"])).sum()
                new_metric.update(key, (pred-gt_reg).abs().item())
            elif "vel_seq" in key:
                try:
                    pred = res[key]
                except:
                    pred = res["vel_seq_final"]
                gt = data["gt_" + key]
                new_metric.update(key, (pred[0]-gt[0][0]).abs().item())
    output = new_metric.compute()
    print(output)
    return output
