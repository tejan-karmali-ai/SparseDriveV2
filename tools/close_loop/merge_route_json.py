...
def merge_route_json(folder_path):
    file_paths = glob.glob(f'{folder_path}/bench2drive*.json')
    merged_records = []
    driving_score = []
    success_num = 0
    ...
    merged_data = {
        ...
        "driving score": sum(driving_score) / length,
        "success rate": success_num / length,
        "eval num": len(driving_score),
    }
    print("driving score:", sum(driving_score) / length)
    print("success rate:", success_num / length)
    print("eval num:", len(driving_score))

    with open(os.path.join(folder_path, 'merged.json'), 'w') as file:
        json.dump(merged_data, file, indent=4)
    
    merged_data.pop("_checkpoint")
    return merged_data
