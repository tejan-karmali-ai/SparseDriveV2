...
# ========== 纯函数，在子进程里执行 ==========
def _process_one_folder(sub_folder, save_root, video_root, records, downsample=4, fps=5):
    """
    ...
    if os.path.exists(dst_video):
        print(f"{sub_folder}: already done")
    ...
    for record in records:
        if record["save_name"] == sub_folder:
            ds = record['scores']['score_composed']
            status = record["status"]
            if "Failed" in status:
                status = "Failed"
            final_mp4 = os.path.join(video_root, f"{status}_{ds}_{sub_folder}.mp4")
...
def images_to_video_parallel(result_dir="close_loop_log",
    ...
    with open(f"{result_dir}/result/merged.json", 'r') as f:
