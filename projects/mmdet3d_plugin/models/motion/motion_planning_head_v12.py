...
from .rescore_utils import interp_anchor_to_traj

@HEADS.register_module()
class MotionPlanningHeadV12(BaseModule):  ## temporal trajectory head
    def __init__(
        self,
        fut_ts=12,
        fut_mode=6,
        ...
        lat_anchor_deformable_model=None,
        ...
        super(MotionPlanningHeadV12, self).__init__()
        self.fut_ts = fut_ts
        ...
        self.op_config_map = {
            ...
            "plan_mode_gnn": [mode_graph_model, ATTENTION],
            ...
            "lat_anchor_deformable": [lat_anchor_deformable_model, ATTENTION],
            "norm": [norm_layer, NORM_LAYERS],
            "plan_mode_norm": [mode_norm_layer, NORM_LAYERS],
        ...
        # plan anchor init
        if lat_anchor is not None:
            lat_anchor = np.load(lat_anchor)
            self.lat_anchor = nn.Parameter(
                torch.tensor(lat_anchor, dtype=torch.float32),
                requires_grad=False,
            )
        else:
            self.lat_anchor = None
        traj_anchor = np.load(plan_config["traj"]["anchor"])
        self.traj_anchor = nn.Parameter(
            torch.tensor(traj_anchor, dtype=torch.float32),
            requires_grad=False,
        )
        # plan mode query
        self.lat_mode_query = nn.Embedding(lat_mode, embed_dims)
        self.lon_mode_query = nn.Embedding(lon_mode, embed_dims)
    ...
    def forward(
        ...
        # =========== plan init ===========
        traj_anchor = torch.tile(self.traj_anchor, (bs, 1, 1, 1))
        if self.lat_anchor is not None:
            lat_anchor = torch.tile(self.lat_anchor, (bs, 1, 1, 1))
        lat_mode_query = torch.tile(self.lat_mode_query.weight, (bs, 1, 1))
        lon_mode_query = torch.tile(self.lon_mode_query.weight, (bs, 1, 1))
        ...
        for i, op in enumerate(self.operation_order):
            if op == "temp_gnn":
                ...
                plan_query = lat_mode_query.unsqueeze(2) + lon_mode_query.unsqueeze(1)
                plan_query = plan_query.flatten(1, 2)
            elif op == "plan_mode_gnn" or op == "plan_mode_norm":
                ...
                plan_anchor_flat = traj_anchor.unflatten(1, (self.lat_mode, self.lon_mode)).mean(dim=2).flatten(-2)
                lat_mode_query = self.layers[i](
                    lat_mode_query,
                    plan_anchor_flat,
                    None,
            ...
            elif op == "lat_anchor_deformable":
                plan_anchor_flat = lat_anchor.flatten(-2)
                lat_mode_query = self.layers[i](
                    lat_mode_query,
                    plan_anchor_flat,
                    None,
    ...
    def loss_planning(self, model_outs, data):
        ...
        for decoder_idx, planning_result in enumerate(planning_results):
            ...
            traj_cls_loss = self.traj_plan_loss_cls(traj_cls, traj_cls_target, traj_cls_weight)
