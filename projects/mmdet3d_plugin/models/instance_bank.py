...
def topk_with_indice(confidence, k, *inputs):
    bs, N = confidence.shape[:2]
    confidence, indices = torch.topk(confidence, k, dim=1)
    ...
    for input in inputs:
        outputs.append(input.flatten(end_dim=1)[indices].reshape(bs, k, -1))
    return confidence, outputs, indices_
...
class InstanceBank(nn.Module):
    ...
    def get(self, batch_size, metas=None, dn_metas=None):
        ...
        # for i in range(100):
        #     da = self.anchor[i].reshape(-1, 2).detach().cpu().numpy()
        #     plt.plot(da[:,0], da[:,1], color=color_mapping[i%len(color_mapping)], linewidth=3, marker='v', linestyle='-', markersize=7)
        # plt.savefig('anchor')
