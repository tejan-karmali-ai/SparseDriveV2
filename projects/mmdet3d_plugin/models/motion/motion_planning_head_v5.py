...
class MotionPlanningHeadV5(BaseModule):
    def __init__(
        self,
        fut_ts=12,
        fut_mode=6,
        ...
        super(MotionPlanningHeadV5, self).__init__()
        self.fut_ts = fut_ts
            ...
            for decoder_idx, planning_result in enumerate(planning_results):
                if "temporal" in key or "spatial" in key:
                    ...
                    if "data_weight" in data:
                        weight = data["data_weight"][:, None]
                    else:
                        weight = None
                    cls_loss = F.binary_cross_entropy(
                        best_cls, gt_cls, weight=weight
                    )
                    ...
                    cls = planning_result[key + "_" + "cls"].sigmoid()
                    gt_cls = data[f"gt_{key}"].float()
                    cls_loss = F.binary_cross_entropy(
                        cls, gt_cls
                    )
                    output[f"planning_{key}_cls_loss_{decoder_idx}"] = cls_loss * value["cls_weight"]
                    ...
                    bs_indices = torch.arange(bs, device=cls.device)
                    best_cls = cls[bs_indices, mode_idx].softmax(dim=-1)
                    gt_vel_seq = data[f"gt_{key}"].unsqueeze(1)
                    gt_vel_seq_mask = data[f"gt_{key}_mask"].unsqueeze(1)
                    anchor = self.plan_anchor[key].data.unsqueeze(0).to(gt_vel_seq.device)
                    ade = (torch.abs(anchor - gt_vel_seq) * gt_vel_seq_mask).mean(dim=-1)
                    gt_cls = ade.argmin(dim=-1)
                    gt_cls = F.one_hot(gt_cls, num_classes=anchor.shape[1]).float()
                    if "data_weight" in data:
                        weight = data["data_weight"][:, None]
                    else:
                        weight = None
