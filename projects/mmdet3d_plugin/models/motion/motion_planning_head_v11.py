...
class MotionPlanningHeadV11(BaseModule):  ## temporal trajectory head
    def __init__(
        self,
        fut_ts=12,
        fut_mode=6,
        ...
        super(MotionPlanningHeadV11, self).__init__()
        self.fut_ts = fut_ts
        ...
        if self.plan_query_mode == "traj":
            self.plan_anchor_encoder = nn.Sequential(
                *linear_relu_ln(embed_dims, 1, 1, ego_fut_ts * 2),
                Linear(embed_dims, embed_dims),
            )
        elif self.plan_query_mode == "random":
            self.plan_mode_query = nn.Embedding(ego_fut_mode, embed_dims)
        elif self.plan_query_mode == "lat_lon":
    ...
    def forward(
        ...
        if self.plan_query_mode == "traj":
            plan_mode_query = self.plan_anchor_encoder(traj_anchor.flatten(-2))
        elif self.plan_query_mode == "random":
            plan_mode_query = torch.tile(self.plan_mode_query.weight, (bs, 1, 1))
        elif self.plan_query_mode == "lat_lon":
        ...
        for i, op in enumerate(self.operation_order):
            if op == "temp_gnn":
                ...
                plan_anchor_flat = traj_anchor.flatten(-2)
                plan_query = self.layers[i](
                    plan_query,
                    plan_anchor_flat,
                    None,
